...
- **Truncation Detection & Repair (LitAgents 3.2)**: After chapter finalization, detects text ending mid-sentence (no final punctuation or last line < 3 words) and auto-repairs via SmartEditor. Enhanced in LitAgents 3.3 with garbled text detection: analyzes word endings across beginning/middle/end text segments to detect AI-corrupted output where words are truncated throughout (>15% words with invalid Spanish endings triggers full rewrite). Also detects "telegram mode" where the AI drops articles, prepositions and conjunctions while keeping content words intact (function word density below 20% vs normal ~40% triggers full rewrite). Also detects "space collapse" where the AI progressively loses spaces between words, fusing them into mega-tokens (>5% of tokens exceeding 25 characters triggers full rewrite). Garbled detection also runs during pre-resume chapter validation.
...
- **Mandatory Plot Thread Closure (LitAgents 3.3)**: Three-layer enforcement ensures all narrative threads have planned resolution chapters. Global Architect must assign `resolution_chapter` for each plot thread (hard-fails if any remain unresolved after auto-fix). World Bible Validator performs code-level verification that resolution chapters exist AND explicitly mention thread closure in summary/key_event, blocking approval otherwise. Resolution chapters are persisted to DB (`plot_threads.resolution_chapter` column) for consistency across runs.
- **World Bible Data Flow Consistency (LitAgents 3.3)**: All World Bible extraction/injection functions support bilingual field names (Spanish legacy: personajes/reglas/ubicaciones/objetos and English GA format: characters/worldRules/locations/objects). `buildFullWorldBibleForRewrite()` includes plotDecisions, plotThreads with resolution_chapter, settings, and themes. Ghostwriter extractors read GA `appearance`/`initial_state` character fields. `buildPlotThreadsContext()` injects resolution chapter info and closure urgency per-thread.
