...
- **LitAgents 2.9.4 (Detect All, Then Fix Strategy)**: New two-phase correction strategy that prevents FinalReviewer inconsistency issues. Phase 1 (Detection): Run 3 consecutive reviews to detect ALL issues, deduplicated into a consolidated registry. Phase 2 (Verified Correction): Fix each issue one by one with immediate verification to ensure no new problems are introduced. If correction verification fails, retry up to 3 times with escalation. Issues that cannot be resolved are marked as "escalated" for manual review. Endpoint: `POST /api/projects/:id/detect-and-fix`. This strategy guarantees a closed list of problems (no "new issues appearing") and prevents regressions. **Cascade Issue System**: When a correction introduces NEW different issues (not the same as the original), the original is marked as resolved and the new issues are added to the registry for later processing. This prevents false escalations where the original issue WAS fixed but the verification failed due to unrelated new problems. Parallel execution protection: Detect & Fix and Final Review cannot run simultaneously.
