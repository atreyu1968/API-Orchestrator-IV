...
- **Truncation Detection & Repair (LitAgents 3.2)**: After chapter finalization, detects text ending mid-sentence (no final punctuation or last line < 3 words) and auto-repairs via SmartEditor. Enhanced in LitAgents 3.3 with garbled text detection: analyzes word endings across beginning/middle/end text segments to detect AI-corrupted output where words are truncated throughout (>15% words with invalid Spanish endings triggers full rewrite). Also detects "telegram mode" where the AI drops articles, prepositions and conjunctions while keeping content words intact (function word density below 20% vs normal ~40% triggers full rewrite). Garbled detection also runs during pre-resume chapter validation.
