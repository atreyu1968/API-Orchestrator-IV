...
- **LitAgents 2.9.5 (Zero-Cascade with Specialized Handling)**: No CASCADE system - corrections never add new issues to the registry. Instead, specialized handling by error type:
  - **Simple errors** (`repeticion_lexica`, `continuidad_fisica`, `color_ojos`): Zero tolerance for new issues - these are word replacements that shouldn't have side effects.
  - **Medium errors** (`ritmo`, `style`, `timeline`, `transicion`): Tolerate 1 minor new issue IF original was fixed. Reject if critical/major new issues or >1 new issues.
  - **Complex errors** (`trama`, `credibilidad_narrativa`, `personajes`, `ubicacion`): Tolerate up to 2 minor new issues IF original was definitely fixed. Reject critical new issues or >2 new issues.
  - Ultra-conservative surgical prompts: max 50 words per patch, no new content, no canonical changes. Patch size validation: patches >100 words or >50% size change are rejected.
